...
extension ViewController: UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if(!Array(DataModel.shared.opportunities)[indexPath.row].value.isScheduled!){
            ...
            if(Array(DataModel.shared.opportunities)[indexPath.row].value.onSale){
                cell.amountLabel?.text = util.doubleFormatter(Array(DataModel.shared.opportunities)[indexPath.row].value.availableBalanceAmount!,0)!+" "+Array(DataModel.shared.opportunities)[indexPath.row].value.currency!
            }else{
                cell.amountLabel?.text = util.doubleFormatter(Array(DataModel.shared.opportunities)[indexPath.row].value.advanceAmount!,0)!+" "+Array(DataModel.shared.opportunities)[indexPath.row].value.currency!
            }
            cell.amountLabel?.adjustsFontSizeToFitWidth = true
            cell.amountLabel?.minimumScaleFactor = 0.5
            cell.ratingLabel?.text = Array(DataModel.shared.opportunities)[indexPath.row].value.evaluation?.rating!
            if(Array(DataModel.shared.opportunities)[indexPath.row].value.isConfirming){
